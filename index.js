...
const webhookPath = process.env.WEBHOOK_PATH?.trim() || "/";
...
bot.command("deploy", async (ctx) => {
    ...
    if (!userId || !(0, auth_1.isAuthorized)(userId)) {
        await ctx.reply("⛔ You are not authorized to run this command.");
        return;
    }
    const statusMsg = await ctx.reply("🚀 Deployment started…");
    try {
        const output = await (0, deploy_1.runDeploy)();
        await ctx.api.editMessageText(ctx.chat.id, statusMsg.message_id, `✅ Deploy finished.\n\n${codeBlock(output)}`, { parse_mode: "Markdown" });
    }
    catch (err) {
        const message = err?.stdout || err?.stderr || err?.message || String(err);
        await ctx.api.editMessageText(ctx.chat.id, statusMsg.message_id, `❌ Deploy failed.\n\n${codeBlock(String(message))}`, { parse_mode: "Markdown" });
    }
});
bot.catch((err) => {
...
async function startWebhookServer() {
    const handleUpdate = (0, grammy_1.webhookCallback)(bot, "stdhttp");
    ...
    await bot.api.setWebhook(webhookUrl, { secret_token: webhookSecretToken || undefined });
